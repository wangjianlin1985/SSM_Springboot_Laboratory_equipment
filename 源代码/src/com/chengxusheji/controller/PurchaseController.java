...
public class PurchaseController extends BaseController {
	...
	public void userAdd(@Validated Purchase purchase, BindingResult br,
		...
		purchase.setUserObj(userObj);
		
		String deviceNo = purchase.getDeviceObj().getDeviceNo();
		Device device = deviceService.getDevice(deviceNo);
		device.setDeviceCount(purchase.getPurchaseCount());
	...
	/*ajax方式按照查询条件分页查询设备采购信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(@ModelAttribute("deviceObj") Device deviceObj,String supplier,String purchaseDate,@ModelAttribute("userObj") UserInfo userObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (supplier == null) supplier = "";
		if (purchaseDate == null) purchaseDate = "";
		if(rows != 0)purchaseService.setRows(rows);
		List<Purchase> purchaseList = purchaseService.queryPurchase(deviceObj, supplier, purchaseDate, userObj, page);
	    /*计算总的页数和总的记录数*/
	    purchaseService.queryTotalPageAndRecordNumber(deviceObj, supplier, purchaseDate, userObj);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("deviceObj") Device deviceObj,String supplier,String purchaseDate,@ModelAttribute("userObj") UserInfo userObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (supplier == null) supplier = "";
		if (purchaseDate == null) purchaseDate = "";
		List<Purchase> purchaseList = purchaseService.queryPurchase(deviceObj, supplier, purchaseDate, userObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    purchaseService.queryTotalPageAndRecordNumber(deviceObj, supplier, purchaseDate, userObj);
	    /*获取到总的页码数目*/
	    int totalPage = purchaseService.getTotalPage();
	...
	public String userFrontlist(@ModelAttribute("deviceObj") Device deviceObj,String supplier,String purchaseDate,@ModelAttribute("userObj") UserInfo userObj,Integer currentPage, Model model, HttpServletRequest request, HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (supplier == null) supplier = "";
		if (purchaseDate == null) purchaseDate = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Purchase/purchase_userFrontquery_result"; 
	}
	...
	public String delete(@PathVariable Integer purchaseId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  purchaseService.deletePurchase(purchaseId);
	            request.setAttribute("message", "设备采购删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "设备采购删除失败!");
				return "error";
	...
	/*按照查询条件导出设备采购信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(@ModelAttribute("deviceObj") Device deviceObj,String supplier,String purchaseDate,@ModelAttribute("userObj") UserInfo userObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(supplier == null) supplier = "";
        if(purchaseDate == null) purchaseDate = "";
        List<Purchase> purchaseList = purchaseService.queryPurchase(deviceObj,supplier,purchaseDate,userObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Purchase信息记录"; 
        String[] headers = { "采购id","采购设备","供应商","采购数量","采购单价","采购日期","采购人"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<purchaseList.size();i++) {
        	Purchase purchase = purchaseList.get(i); 
        	dataset.add(new String[]{purchase.getPurchaseId() + "",purchase.getDeviceObj().getDeviceName(),purchase.getSupplier(),purchase.getPurchaseCount() + "",purchase.getPrice() + "",purchase.getPurchaseDate(),purchase.getUserObj().getName()});
        }
        /*
